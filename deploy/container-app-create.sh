...
# Script to create Azure Container App for sautAI Frontend
set -e
...
# Using Azure Key Vault secrets (sautai-keyvault)
# No environment variables needed - all secrets come from Key Vault
...
if az containerapp show --name $APP_NAME --resource-group $RESOURCE_GROUP >/dev/null 2>&1; then
    ...
    az containerapp create \
        ...
        --ingress external \
        --secrets \
            django-url=keyvaultref:https://$KEY_VAULT_NAME.vault.azure.net/secrets/django-url,identityref:/subscriptions/$(az account show --query id -o tsv)/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.ManagedIdentity/userAssignedIdentities/django-keyvault-identity \
            openai-api-key=keyvaultref:https://$KEY_VAULT_NAME.vault.azure.net/secrets/openai-api-key,identityref:/subscriptions/$(az account show --query id -o tsv)/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.ManagedIdentity/userAssignedIdentities/django-keyvault-identity \
        --env-vars \
